...
- **SRS-UX-04**: Кабинеты мерчанта и вендора **должны** использовать единый Tailwind-макет с фиксированным navbar, адаптивными таблицами и мобильной навигацией, соответствующей админским страницам.

### 3.10. QR-оплата (Self-Service Purchase)
- **SRS-QR-01**: Система **должна** позволять админу генерировать уникальные ссылки для пары Мерчант+Товар через интерфейс `/admin/qr-links`.
- **SRS-QR-02**: Каждая ссылка **должна** иметь уникальный `token` (UUID) и храниться в `MerchantProductLink`.
- **SRS-QR-03**: Система **должна** генерировать QR-код (PNG) для скачивания и отображения на странице.
- **SRS-QR-04**: Публичная страница `/pay/:token` **должна** отображать информацию о товаре (название, цена, мерчант) без авторизации.
- **SRS-QR-05**: Перед отображением страницы система **должна** проверять:
  a) `MerchantProductLink.isActive = true`;
  b) `Product.status = 'on'`;
  c) `Merchant.status = 'active'`;
  d) наличие хотя бы одного `Voucher` со статусом `active` для данного товара.
- **SRS-QR-06**: Страница checkout `/pay/:token/checkout` **должна** требовать ввод номера телефона (+998XXXXXXXXX) и выбор способа оплаты (Click/Payme).
- **SRS-QR-07**: При отправке формы система **должна** создать `QrPaymentAttempt` со статусом `PENDING` и `expiresAt` = текущее время + 2 минуты.
- **SRS-QR-08**: Система **должна** генерировать корректные URL для редиректа на платёжные системы Click и Payme с подписью и параметрами.
- **SRS-QR-09**: При получении успешного callback от Click/Payme система **должна**:
  a) обновить `QrPaymentAttempt.status` → `PAID`;
  b) выбрать первый доступный `Voucher` (статус `active`) и перевести в `sold`;
  c) создать/найти `Client` по номеру телефона;
  d) создать `Sale` с `saleType = ONLINE`;
  e) создать `VoucherTransaction` и обновить балансы;
  f) создать `OnlineVoucher` для привязки ваучера к клиенту;
  g) сгенерировать PDF-чек и сохранить путь в `Sale.receiptPath`;
  h) отправить SMS с ваучером на указанный номер.
- **SRS-QR-10**: Страница результата `/pay/:token/result/:attemptId` **должна** отображать электронный чек с полным кодом ваучера, кнопкой скачивания PDF и ссылкой на кошелёк.
- **SRS-QR-11**: Система **должна** иметь cron-задачу для пометки просроченных `QrPaymentAttempt` (статус `PENDING` и `expiresAt < now()`) как `EXPIRED`.
- **SRS-QR-12**: В случае отсутствия ваучеров в статусе `active` во время callback система **должна** пометить попытку как `FAILED` и не создавать `Sale`.
